...
fn bincode_opts() -> impl bincode::Options + Copy {
    bincode::options()
        .with_no_limit()
}
...
async fn main() -> io::Result<()> {
    ...
    let mut transport = tarpc::serde_transport::tcp::listen("localhost:0", || Bincode::from(bincode_opts())).await?;
    ...
    let transport = tarpc::serde_transport::tcp::connect(addr, Bincode::from(bincode_opts())).await?;
